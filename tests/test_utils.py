...
from tests import TEST_DIRECTORY, TEST_DATA_DIRECTORY, ROOT_DIRECTORY
import pandas as pd
from mongoie.chunky import chunk_generator, ChunkedDataStream
...
@pytest.mark.parametrize(
    ...
    [(0, 100, 20), (0, 1000, 300), (1, 100, 1), (5, 5, 5), (0, 1, 1)],
)
...
def test_should_remove_last_character():
    ...
    data = [{"a": "b"}]
    file_path = os.path.abspath(os.path.join(TEST_DIRECTORY, file_path))
...
def test_should_properly_json_normalize():
    docs = [{"name": "John", "address": {"country": "Spain", "city": "Barcelona"}}]
    data = json_normalize(docs)
    assert isinstance(data, pd.DataFrame) and set(data.columns) == {
        "name",
        "address.country",
        "address.city",
    }
...
def test_should_properly_df_denormalize():
    ...
    assert df_denormalize(data) == [
        {"address": {"city": "Barcelona", "country": "Spain"}, "name": "John"}
    ]
...
@pytest.mark.parametrize(
    "fp, exc",
    [
        (os.path.join(TEST_DIRECTORY, r"data\data.json"), None),
        (5, TypeError),
...
@pytest.mark.parametrize(
    "file_path, file_extension, result",
    [
        (r"file\file.csv", "csv", r"file\file.csv"),
        (r"file\file", "json", r"file\file.json"),
        (r"file\file", ".json", r"file\file.json"),
        (
            r"long1\long2\long3\file\file",
            ".parquet",
            r"long1\long2\long3\file\file.parquet",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            r"long\long\data\file.json",
            "__suffix__",
            r"long\long\data\file___suffix__.json",
        ),
    ],
)
def test_should_properly_add_number_prefix_to_the_file_path(
    test_directory, fp, suffix, output
):
    path_with_suffix = add_number_prefix_to_file_path(fp, suffix)
    assert str(path_with_suffix) == os.path.join(ROOT_DIRECTORY, output)
